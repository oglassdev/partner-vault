...
export default function ProfileCreationForm(props: { onComplete: () => void }) {
    ...
    const handleSubmit: SubmitHandler<ProfileForm> = async (values) => {
        ...
        let { error }  = await supabase
            ...
            .insert({
                id: session.user.id,
                public_email: values.public_email.length == 0 ? null : values.public_email,
    ...
    return <Form onSubmit={handleSubmit} class="gap-3 flex flex-col my-auto">
        ...
        <Field name="public_email">
            {(_, props) => (
                <Input {...props} type="email" placeholder="Leave blank to make your email private" />
            )}
        </Field>
        <Button type="submit" disabled={profileForm.submitting}>
            {profileForm.submitting && <TbLoader class="mr-2 h-4 w-4 animate-spin" />}
            Update Profile
        </Button>
    </Form>
}