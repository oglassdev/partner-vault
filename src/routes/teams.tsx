...
export default function Teams() {
    ...
    return <main class="flex flex-col p-4 w-full h-full gap-2">
        <header class="mt-4 flex flex-row gap-2">
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <As component={Button} variant="outline" size="icon" class="flex-none">
        ...
        <Suspense fallback={<Loader />}>
            <Show when={viewType() == "grid"} fallback={
                                ...
                                {team => (
                                    <TableRow>
                                        ...
                                        <TableCell class="gap-2 flex justify-end">
                                            <Button variant="outline" class="flex-none">
                                                Select Team
                                            </Button>
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                ...
                                                <DropdownMenuContent>
                                                    <DropdownMenuItem>
                                                        <span class="w-full text-center text-red-500 font-medium">Leave Team</span>
                                                    </DropdownMenuItem>
                                                </DropdownMenuContent>
                                            </DropdownMenu>
                ...
                <div class="flex flex-wrap flex-row overscroll-auto overflow-auto w-full gap-2">
                    <For each={teams()}>
                        {team => {
                            const {name} = team;
                            return <Card class="w-56 h-auto flex flex-col">
                                <CardHeader class="p-4 flex flex-row">
                                    <CardTitle>{name}</CardTitle>
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <As component={Button} variant="ghost" size="icon" class="flex-none ml-auto w-9 h-9">
                                ...
                                <CardFooter class="flex flex-row gap-2 p-4 pt-0 mt-auto">
                                    <Button variant="outline" class="flex-auto">
                                        Select Team
                                    </Button>
                                </CardFooter>
                            </Card>
        ...
        <footer class="w-full flex flex-row mt-auto gap-1 items-center">
            <ColorModeToggle/>
            <Help>If you aren't in a team, you can either create one or ask for an invite.</Help>
            <LogoutButton />
            <span class="mx-auto"/>
            <Invites/>
        </footer>
